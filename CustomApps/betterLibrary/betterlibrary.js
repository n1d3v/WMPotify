...
function enableBetterLib() {
    waitForElm('.betterLibBox').then((elm) => {
        if (!betterLibIsEnabled)
        {
            ...
            lastPanelWidth = document.documentElement.style.getPropertyValue("--panel-width");
            ...
            setTimeout(() => {
                document.documentElement.style.setProperty("--panel-width", lastPanelWidth);
            }, 100);
...
function disableBetterLib() {
    if (betterLibIsEnabled)
    {
        ...
        Spicetify.Platform.LocalStorageAPI.setItem("ylx-sidebar-state",localStorage.wmpotifyShowLibX ? lastSidebarMode : 1);
